    ...
    import json key_dict = json.loads(st.secrets["textkey"])
   ...
   if btnFiltrar: doc = loadByName(nameSearch) 
   if doc is None:   
 ...
 if btnEliminar: 
   ...
   if btnActualizar: updatename = loadByName(nameSearch) 
       if updatename is None:   st.write(f"{nameSearch} no existe") 
       else:  
